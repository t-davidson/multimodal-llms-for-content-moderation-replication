...
async def compare_single_image_async(client, rate_limiter, image_index, local_dir="../../synthetic_posts/main/"):
    ...
    try:
        ...
        # Load image
        image_bytes = await load_image_bytes(image_path)
        
        if image_bytes is None:
            return image_index, f"Error: Image not found ({image_path})"
        ...
        response = await client.aio.models.generate_content(
            ...
            config=types.GenerateContentConfig( 
                thinking_config=types.ThinkingConfig(thinking_budget=0), # Ensures thinking is not active
                max_output_tokens=1,
                temperature=0
            )
        )
        
        return image_index, response.text.strip()
...
async def main():
    ...
    try:
        ...
        print(f"Loaded {len(a_images)} images from {file_path}")
    except Exception as e:
    ...
    rate_limiter = RateLimiter(max_requests_per_minute=3950)
    
    # Setup output
    output_file = "results/single_results_gemini_2_5_flash_lite_async.csv"
    ...
    if os.path.exists(output_file):
        try:
            ...
            print(f"Resuming from {len(processed_indices)} already processed images")
        except Exception as e:
    ...
    for idx, image_index in enumerate(a_images, start=1):
        if idx not in processed_indices:
            remaining_images.append((idx, image_index))
    
    if not remaining_images:
        print("All images already processed!")
        return
    
    print(f"Processing {len(remaining_images)} remaining images using Gemini 2.5 Flash Lite")
    print(f"Rate limit: 950 requests/minute")
    print(f"Expected completion time: ~{len(remaining_images)/950:.1f} minutes")
    print("=" * 60)
    ...
    async def process_single_image(idx, image_index):
        """Process a single image with semaphore control"""
        async with semaphore:
            return idx, await compare_single_image_async(client, rate_limiter, image_index)
    
    # Create all tasks
    tasks = [process_single_image(idx, image_index) for idx, image_index in remaining_images]
    ...
    for chunk_idx in range(0, len(tasks), chunk_size):
        ...
        try:
            ...
            for result in chunk_results:
                ...
                try:
                    idx, (image_index, output) = result
                    results.append([idx, image_index, output])
            ...
            try:
                df = pd.DataFrame(results, columns=["Index", "Image", "Output"])
            ...
            print(f"Total progress: {total_processed}/{len(remaining_images)} images ({total_processed/len(remaining_images)*100:.1f}%)")
            
            if total_time > 0 and total_processed > 0:
                rate = total_processed / total_time * 60
                print(f"Average rate: {rate:.1f} images/minute")
                
                remaining = len(remaining_images) - total_processed
                if remaining > 0:
    ...
    print(f"Images processed: {final_processed}/{len(remaining_images)}")
    
    if total_time > 0 and final_processed > 0:
        print(f"Average rate: {final_processed/(total_time/60):.1f} images/minute")
