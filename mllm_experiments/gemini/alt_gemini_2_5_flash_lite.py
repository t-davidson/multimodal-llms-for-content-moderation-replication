...
async def compare_images_async(client, rate_limiter, index_a, index_b, path_a, path_b):
    """
    Compare two images using Gemini 2.5 Flash Lite - async version
    ...
    try:
        ...
        # Generate content using Gemini 2.5 Flash Lite (note additional argument for thinking)
...
async def main():
    """Main async function"""
    # Load dataset
    file_path = '../gpt4o/image_indices_alt_99750.csv'
    ...
    try:
        ...
        print(f"Loaded {len(image_data)} image pairs from {file_path}")
    except Exception as e:
    ...
    # Setup output
    output_file = "results/alt_results_gemini_2_5_flash_lite_async.csv"
    ...
    for idx, data in enumerate(image_data, start=1):
        if idx not in processed_indices:
            ...
            remaining_pairs.append((idx, (index_a, index_b, path_a, path_b)))
    
    if not remaining_pairs:
        print("All pairs already processed!")
        return
    
    print(f"Processing {len(remaining_pairs)} remaining pairs using Gemini 2.5 Flash Lite")
    ...
    async def process_single_pair(idx, data):
        """Process a single pair with semaphore control"""
        async with semaphore:
            index_a, index_b, path_a, path_b = data
            return idx, await compare_images_async(client, rate_limiter, index_a, index_b, path_a, path_b)
    
    # Create all tasks
    tasks = [process_single_pair(idx, data) for idx, data in remaining_pairs]
